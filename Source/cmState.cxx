...
cmState::Traverse(int depth, PositionType it) const
{
  ...
  for (std::map<cmListFileContext, std::vector<cmState::Snapshot> >::const_iterator
       cit = childSnapshots.begin(); cit != childSnapshots.end(); ++cit)
    {
  ...
  while(ns.IsValid())
    ...
    for (std::map<cmListFileContext, std::vector<cmState::Snapshot> >::const_iterator
         cit = newSnapshots.begin(); cit != newSnapshots.end(); ++cit)
      {
      std::vector<cmState::Snapshot>& vec = snapshots[cit->first];
      vec.insert(vec.end(), cit->second.begin(), cit->second.end());
      }
...
std::map<cmListFileContext, std::vector<cmState::Snapshot> > cmState::TraceSnapshots() const
{
...
std::vector<cmState::Snapshot>
cmState::GetWriters(cmState::Snapshot snp, std::string varName) const
{
...
void cmState::Snapshot::UnmarkNotExecuted(long begin)
{
  ...
  std::map<long, long>::iterator nx =
      nxData.NotExecuted.find(begin);
...
void cmState::Snapshot::MarkNotExecuted(long begin, long end)
{
  auto& nxData =
      this->State->NotExectutedLines[*this->Position->ExecutionListFile];
  if (nxData.Executed.find(begin)
      != nxData.Executed.end())
    {
    return;
    }
  nxData.NotExecuted.insert(
        std::make_pair(begin, end));
}

std::map<long, long>
cmState::GetNotExecuted(std::string const& path) const
{
