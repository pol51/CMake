...
void cmServerParser::Add(cmListFileLexer_Token* token, std::vector<Token>& value,
                         DifferentialFileContent diff,
                         const char* name, cmCommand* cmd,
                         std::vector<std::string> const& args)
{
  ...
  Token t;
  if (!cmd || args.empty()) {
    t.Type = name ? name : cmListFileLexer_GetTypeAsString(this->Lexer, token->type);
    }
  else
    {
  ...
  for (auto const& ch: diff.Chunks)
    ...
    if (ch.NumAdded != 0)
      ...
      if (ch.NewStart > tokens.front().Line)
        {
        auto lb = std::lower_bound(tokens.begin(), tokens.end(), ch.NewStart,
                         [](Token const& lhs, long rhs)
          {
          return lhs.Line < rhs;
          });
        ...
        auto itToExecFrom = std::lower_bound(diff.Chunks.begin(),
                                             diff.Chunks.end(),
                                             fileStartLine,
                         [](Chunk const& lhs, long rhs) {
          return lhs.OrigStart < rhs;
        });
        if (itToExecFrom == diff.Chunks.end() || itToExecFrom->OrigStart != fileStartLine)
          {
          --itToExecFrom;
          }
...
void cmServerParser::Scan(DifferentialFileContent diff, std::vector<Token>& ret)
{
