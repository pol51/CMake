...
cmGetPropertyCommand::GetContextForParameter(const std::vector<std::string>& args, size_t index)
{
  if (index == 0)
  ...
  if (index == 2 && ((args.size() > 2 && args[1] == "TARGET") || args.size() == 2))
    {
      return SingleBinaryTargetParameter;
    }
  if (index == 2 && args[1] == "GLOBAL" && ((args.size() > index && args[index] == "PROPERTY") || args.size() == 2))
    return KeywordParameter;
  if (index == 3 && ((args.size() > index && args[index] == "PROPERTY") || args.size() == 3))
    return KeywordParameter;
...
cmGetPropertyCommand::GetKeywords(const std::vector<std::string>& args,
                                  size_t index)
{
  (void)args;
  (void)index;
